...
def saliency_mask(model, F, Q, L, iter_rounds, lr):
    
    tv_beta = 3
    learning_rate = lr
    max_iterations = iter_rounds
    l1_coeff = 0.5

    mask_init = np.ones(F.shape, dtype=np.float32)

    mask = numpy_to_torch(mask_init, requires_grad=True)
    optimizer = torch.optim.Adam([mask], lr=learning_rate)

    for i in range(max_iterations):
        (img_feats, label, input_ids, input_mask, segment_ids) = eval_dataset.raw2tensor(F.mul(mask), Q, L)
...
def words_saliency(model, F, Q, L):
...
def main():
    ...
    parser.add_argument('--fuzz_lr', type=float)
    parser.add_argument('--fuzz_epoch', type=int)
    ...
    for i, image_name in enumerate(image_list):
        ...
        image_item_list.append({
            ...
            'target_A': L,
            'target_Q': target_Q,
            'image_feature': F.detach().cpu().clone(),
            })
    progress.finish()
    ...
    for _iter in range(exp_length):
        ...
        F = image_item['image_feature'].cuda()
        target_Q = image_item['target_Q']
        min_F = torch.min(F)
        max_F = torch.max(F)
        orig_F = F.clone().detach()
        orig_mask, _ = saliency_mask(model, orig_F, target_Q, L, args.fuzz_epoch, args.fuzz_lr)
        for Q in Q_list:
            temp_F = F.clone().detach()
            orig_score = words_saliency(model, temp_F, Q, L)
            new_mask, pred_L = saliency_mask(model, temp_F, Q, L, args.fuzz_epoch, args.fuzz_lr)
            if pred_L == L:
                mask = 1 + (1 - new_mask) * 0.3
            else:
                mask = (1.0 - orig_mask.mul(1.0 - new_mask)) + (1 - orig_mask) * 0.3
            try:
                temp_F = temp_F.mul(mask)
                # temp_F.data.clamp_(min_F, max_F)
            except:
                temp_F = temp_F.mul(mask)
            new_score = words_saliency(model, temp_F, Q, L)
            if dominated_rate(model, Q_list, temp_F, L) > image_item['acc'] and new_score <= orig_score:
                F = temp_F
                orig_mask, _ = saliency_mask(model, temp_F, target_Q, L, args.fuzz_epoch, args.fuzz_lr)
        ...
        if new_d == 1:
            ...
            del image_item_list[idx]
            print(' Found: %s' % image_item['image_name'])
        elif new_d > image_item['acc']:
            image_item['acc'] = new_d
            image_item['image_feature'] = F.cpu().clone()
            image_item_list[idx] = image_item
            image_item_list = sorted(image_item_list, key=sort_key, reverse=True)
            # print(' UP UP :D')
        elif new_d == image_item['acc']:
            image_item['times'] += 1
            image_item['image_feature'] = F.cpu().clone()
            if image_item['times'] > 4:
                del image_item_list[idx]
        else:
            image_item['times'] += 1
            if image_item['times'] > 4:
                del image_item_list[idx]
        del orig_F, F, temp_F, orig_mask, mask, new_mask
        if len(image_item_list) < 1:
            break
    progress.finish() 
