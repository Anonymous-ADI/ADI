...
class CaptionBertSelfAttention(BertSelfAttention):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,
            history_state=None, debug=False):
...
class CaptionBertAttention(BertAttention):
    ...
    def forward(self, input_tensor, attention_mask, head_mask=None,
            history_state=None, debug=False):
        self_outputs = self.self(input_tensor, attention_mask, head_mask, history_state, debug=debug)
...
class CaptionBertLayer(BertLayer):
    ...
    def forward(self, hidden_states, attention_mask, head_mask=None,
        ...
        attention_outputs = self.attention(hidden_states, attention_mask,
                head_mask, history_state, debug=debug)
...
class BertImgModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        self.word_inputs = embedding_output.detach()
        self.word_inputs.requires_grad = True
        ...
        if img_feats is not None:
            ...
            embedding_output = torch.cat((self.word_inputs, img_embedding_output), 1) # 128 + 50 --> 178
...
class ImageBertForSequenceClassification(BertPreTrainedModel):
    ...
    def forward(self, input_ids_list, token_type_ids_list=None, attention_mask_list=None, labels=None, 
            position_ids=None, head_mask=None, img_feats_list=None, is_feats_list=False):
        outputs_list = []
        input_ids_list = input_ids_list.transpose(0, 1)
        token_type_ids_list = token_type_ids_list.transpose(0, 1)
        attention_mask_list = attention_mask_list.transpose(0, 1)
        if not is_feats_list:
            img_feats = img_feats_list
        else:
            img_feats_list = img_feats_list.transpose(0, 1)
        for idx in range(len(input_ids_list)):
            if is_feats_list:
                img_feats = img_feats_list[idx]

            input_ids = input_ids_list[idx]
            token_type_ids = token_type_ids_list[idx]
        ...
        return torch.stack(outputs_list, 0).transpose(0, 1) # bs, seq_len

class ImageBertForSequenceClassificationCopy(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ImageBertForSequenceClassificationCopy, self).__init__(config)
        self.num_labels = config.num_labels
        self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
    ...
    def forward(self, input_ids_list, token_type_ids_list=None, attention_mask_list=None, labels=None, 
            position_ids=None, head_mask=None, img_feats_list=None, is_train_cls=False, has_noise=None):
        outputs_list = []
        result_list = []
        final_feat_list = []
        input_ids_list = input_ids_list.transpose(0, 1)
        ...
        for idx in range(len(input_ids_list)):
            ...
            if np.random.uniform() < has_noise:
                pooled_output = torch.randn(outputs[1].size()).clamp(-1, 1).cuda()
            else:
                pooled_output = outputs[1]
            #print('pooled output: ', pooled_output.shape)
            final_feat_list.append(pooled_output)
            pooled_output = self.dropout(pooled_output)
            #print('pooled output: ', pooled_output.shape)
            logits = self.classifier(pooled_output)
            ...
            #print(outputs)
            # outputs: [classifier(pooled_output), encoder_outputs[1:]]
            if labels is not None:
                if self.num_labels == 1: #  doing regression
                    loss_fct = MSELoss()
                    labels = labels.to(torch.float)
        ...
        # seq_len, bs
        #if is_train:
            #return MTlabel, torch.stack(final_feat_list, 0).transpose(0, 1)
        return torch.stack(outputs_list, 0).transpose(0, 1), torch.stack(final_feat_list, 0).transpose(0, 1)
        #return torch.stack(outputs_list, 0).transpose(0, 1) # bs, seq_len

class ImageBertForSequenceClassificationOriginal(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ImageBertForSequenceClassificationOriginal, self).__init__(config)
        self.num_labels = config.num_labels
        self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
...
class ImageBertForSequenceClassificationBP(BertPreTrainedModel):
    """
    Modified from BertForSequenceClassification to support oscar training.
    """
    def __init__(self, config):
        super(ImageBertForSequenceClassificationBP, self).__init__(config)
        self.num_labels = config.num_labels
        self.loss_type = config.loss_type
        self.config = config
        if config.img_feature_dim > 0:
    ...
    def init_code_embedding(self, em):
        self.bert.code_embeddings.weight.data = em.clone()
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, 
            position_ids=None, head_mask=None, img_feats=None, no_label=None):

        outputs = self.bert(input_ids, position_ids=position_ids, token_type_ids=token_type_ids,
                            attention_mask=attention_mask, head_mask=head_mask, img_feats=img_feats)
        self.word_inputs = self.bert.word_inputs
        pooled_output = outputs[1]
            ...
            if self.num_labels == 1: #  doing regression
                ...
                if self.loss_type == 'kl':
                    ...
                    if no_label:
                        ...
                        pred_L = idx[0].item()
                        new_labels = torch.LongTensor([pred_L]).unsqueeze(1).expand(1, 3129).type(torch.FloatTensor).cuda()
                        loss = instance_bce_with_logits(logits, new_labels)
                    else:
                        loss = instance_bce_with_logits(logits, labels)
                else:
                    loss_fct = CrossEntropyLoss()
                    loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
            outputs = (loss,) + outputs + (self.word_inputs,)
        return outputs
